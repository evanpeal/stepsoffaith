  ...
  function App(){
    ...
    async function load(){
      try {
        ...
        setState(res && res.value ? JSON.parse(res.value) : { completed: [], completedCheckpoints: [], streak: 0, gems: 0, dailyStreak: 0, lastCheckIn: null, claimedQuests: [], profile: { name: 'Your name', avatar: '\ud83d\udcd6' }, testimony: '', reflections: [], testBest: {}, deepStudies: [] });
      } catch (ex) { setState({ completed: [], completedCheckpoints: [], streak: 0, gems: 0, dailyStreak: 0, lastCheckIn: null, claimedQuests: [], profile: { name: 'Your name', avatar: '\ud83d\udcd6' }, testimony: '', reflections: [], testBest: {}, deepStudies: [] }); }
    }
    async function persist(next){ setState(next); try { await window.storage.set(KEY, JSON.stringify(next), false); } catch (ex) {} }
